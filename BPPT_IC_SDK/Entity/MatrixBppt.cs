...
namespace BPPT_IC_SDK.Entity
{
    ...
    {
        public static MyImage Mat_FloatToChar(MyImage mat)
        {
        ...
        public static MyImage Mat_Invert(MyImage src, MyImage dst)
        {
            int i, j;
            if (dst == null)
            {
        ...
        public static MyImage Mat_FlipV(MyImage src, MyImage dst)
        {
            int i, j;
            if (dst == null)
            {
            ...
            for (j = 0; j < src.Height; j++)
                for (i = 0; i < src.Width; i++)
                    dst.UPixel[src.Height - 1 - j, i] = src.UPixel[j, i];
            return dst;
        }

        public static MyImage Mat_Clone(MyImage myimage)
        {
            ...
            else if (myimage.TypeData == MyImgType.DOUBLE)
            {
                System.Array.Copy(myimage.DPixel, newMat.DPixel, newMat.DPixel.Length);
            }
